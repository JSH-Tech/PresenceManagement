...
export const ajoutEmploye = async (req, res) => {
    ...
    try {
        ...
        const employe = await Employes.create({nomEmploye,prenomEmploye,dobEmploye,emailEmploye,departementEmploye,passwordEmploye: hashPassword,idType_Employe,
        });
...
export const modifierEmploye = async (req, res) => {
    ...
    try {
        const employeAModifier = await Employes.findByPk(idEmploye);
        ...
        await employeAModifier.update(nouvellesInfosEmploye);
        res.status(200).json({ message: "Employé modifié avec succès", data: employeAModifier });
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
};
...
export const supprimerEmploye = async (req, res) => {
    ...
    try {
        const employeASupprimer = await Employes.findByPk(idEmploye);

        if (!employeASupprimer) {
            return res.status(404).json({ message: "Employé non trouvé" });
        }

        await employeASupprimer.destroy();
...
export const listEmployesParType = async (req, res) => {
    const idType_Employe = parseInt(req.params.id);

    try {
        const employes = await Employes.findAll({ where: { idType_Employe } });
        res.status(200).json({ data: employes });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
